//! Canonical stress-test shock calculation shared by `src-tauri` and `portfolio-mcp`.
//!
//! A TypeScript port of this logic lives in `frontend/lib/scenarioMath.ts` for use
//! when no Tauri backend is available (browser dev mode, scenario-comparison
...
mod tests {
    ...
    // ── Cross-language parity guard (#646) ──────────────────────────────────
    // Mirrors the "cross-language parity" test in
    // frontend/lib/__tests__/scenarioMath.test.ts. Keep the input scenario and
    // expected numbers identical in both places if either implementation changes.
    #[test]
    fn parity_matches_typescript_scenario_math() {
        let snapshot = make_snapshot(vec![
            make_holding("AAPL", AssetType::Stock, "USD", 10_000.0),
            make_holding("RY.TO", AssetType::Stock, "CAD", 5_000.0),
        ]);
        let mut shocks = HashMap::new();
        shocks.insert("stock".to_string(), -0.20);
        shocks.insert("fx_usd_cad".to_string(), 0.05);
        ...
        assert!((result.holding_breakdown[0].stressed_value - 8_400.0).abs() < 0.000_001);
        // RY.TO: 5000 * 0.80 = 4000 (CAD holding ignores fx_usd_cad)
        assert!((result.holding_breakdown[1].stressed_value - 4_000.0).abs() < 0.000_001);
        assert!((result.stressed_value - 12_400.0).abs() < 0.000_001);
        assert!((result.total_impact - (-2_600.0)).abs() < 0.000_001);
        assert!((result.total_impact_percent - (-17.333_333_333_333_332)).abs() < 0.000_001);
    }
}
